...
Самый простой способ перенести пользователей из базы безопасности Firebird 3.0 в базу данных безопасности
Firebird 4.0 заключается резервной копии `security3.fdb` с помощью _gbak_ и восстановлении её в Firebird 4.0.
